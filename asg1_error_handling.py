...
class Figure:
    ...
    def create_mask_view(self, colors, fill_value):
        ...
        mask_view = self.ax.imshow(mask_data, interpolation='nearest', interpolation_stage='rgba', cmap=cmap, norm=norm)
...
class KmeansPresenter:
    ...
    def on_key_down(self, e): # do the following when certain "key" is pressed
        if e.key == 'i':    # make 1 iteration of K-means
            ...
            self.fig.draw()
        if e.key == 'o':    # run K-means to convergence (when SSE (energy) improvement is less than given threshold)
            delta = 1.0
            while (delta > 0.001):
            ...
            self.fig.set_mask_view_data(self.region_mask_view_id, region_mask, self.colors)
            self.fig.draw()
        if e.key == 'p':    # run K-means to convergence with visualization of each iteration
            delta = 1.0
            while (delta > 0.001):
                delta = self.alg.compute_k_means_clusters()
                region_mask = self.alg.get_region_mask()
                ...
                self.fig.set_mask_view_data(self.region_mask_view_id, region_mask, self.colors)
                self.fig.draw()
        if e.key == 'w':    # restart K-means (with random clusters)
            self.alg.init_means()
            self.alg.compute_k_means_clusters()
            region_mask = self.alg.get_region_mask()
            if (self.palette == 'mean'):
        ...
        if e.key == 't':    # change colors to some Transparent colors (randomly selected)
            ...
            self.fig.set_mask_view_data(self.region_mask_view_id, region_mask, self.colors)
            self.fig.draw()
        if e.key == 'e':    # changes colors to average (mean) RGB color in each segment
            region_mask = self.alg.get_region_mask()
            self.set_colors('mean')
            self.fig.set_mask_view_data(self.region_mask_view_id, region_mask, self.colors)
            self.fig.draw()
